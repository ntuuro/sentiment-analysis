...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    t = feature_vector@theta + theta_0
    return max(0,1 - t * label)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            theta, theta_0 =  perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            thetas.append(theta)
    return np.mean(thetas, axis = 1), theta_0
...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    # x = feature_vector, y = label, T = Lambda, L = eta
    if (feature_vector@current_theta + current_theta_0) * label  <= 1:
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    # Your code here
    # x = feature_matrix, y = labels, T = Lambda, L = eta
    # theta = np.zeros(feature_matrix.shape[1])
    # theta_0 = 0
    ...
    #         if  (feature_matrix[j]@theta + theta_0) * labels[j]  <= 1:
    #             # Update the weights of the model
    #             theta = (1 - eta * L )* theta + eta * labels[j] * feature_matrix[j]
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # predicts = np.zeros(feature_matrix.shape[0])
    # for i in get_order(feature_matrix.shape[0]):
    #     feature_vector = feature_matrix[i]
    #     prediction = (feature_vector@theta) + theta_0
    #     if (prediction > 0):
    #         predicts[i] = 1
    #     else:
    #         predicts[i] = -1
...
def classifier_accuracy(
    ...
    val_accuracy = accuracy(val_predict, val_labels)
